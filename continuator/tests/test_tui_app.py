...
def test_tui_welcome_and_briefing_display() -> None:
    async def run() -> None:
        ...
        async with app.run_test(size=(120, 45)) as pilot:
            assert TITLE_PLAIN_SIGNATURE in pilot.app.query_one("#hero-logo").render().plain
            assert "Turn conversations into AI continuation briefings." in (
                pilot.app.query_one("#hero-tagline").render().plain
            )
            assert "Continue" in pilot.app.query_one("#hero-actions").render().plain
